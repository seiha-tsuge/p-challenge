...
実際には存在しないレコード間の「隙間（ギャップ）」に対するロック。
...
トランザクションAの2回目のクエリは、トランザクションBによって追加された新しいレコードを含む異なる結果を返します。
...
sequenceDiagram
    ...
    participant ReservationAPI as 予約API
    ...
    alt 席が既に購入されている場合
        ...
        Terminal->>ReservationAPI: 予約ロックを要求
        ReservationAPI-->>Terminal: 予約ロック確認
        Terminal->>Database: 最新のバージョンを再確認
        Database-->>Terminal: 最新のバージョン情報
        alt バージョンが一致しない場合
            ...
            alt バージョンが一致しない場合
                ...
                Terminal->>PaymentAPI: 返金処理を行う
                PaymentAPI-->>Terminal: 返金完了
                Terminal-->>User: エラー: 予約に失敗しました
            else バージョンが一致する場合
