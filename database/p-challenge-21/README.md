...
顧客が支払いを完了した後にデータベースの更新に失敗すると、支払い済みでもチケットが手に入らない問題が生じます。これを防ぐため、決済処理の前にまずデータベースを更新します。この方法では、データベース更新の失敗があれば、決済前にエラーを検出できます。もし決済が失敗した場合、データベースは更新前の状態に戻せます。
