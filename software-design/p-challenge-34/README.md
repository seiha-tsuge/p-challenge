...
システム上で何かを描画するときは、複数の集約にアクセスして情報を取り出し、UI層に受け渡す。その際の方法として、複数の集約の情報を「データ変換オブジェクト(DTO)」に詰め替える方法が挙げられる。この場合、まずアプリケーションは、リポジトリ経由で複数の集約を取得する。次に、DTOに値を設定する。
...
以下の特徴の説明は、「エンティティ」それとも「値オブジェクト」、どちらの説明でしょうか。

| No  | 特徴                 | 説明                                               |
| --- | -------------------- | -------------------------------------------------- |
| 1   | 計測/定量化/説明     | ドメイン内の知識を表現(計測/定量化/説明)する       |
| 2   | 不変                 | インスタンスを生成した後は、値は変化しない         |
| 3   | 概念的な統一体       | 関連する属性を不可欠な単位として組み合わせる       |
| 4   | 交換可能性           | 値が変わるときには、全体を置き換えられる           |
| 5   | 等価性               | 値が等しいかどうかを、他のインスタンスと比較できる |
| 6   | 副作用のない振る舞い | 何回呼び出しても、オブジェクトの状態が変わらない   |

### クイズ2

ドメインサービスの説明は1か2どちらでしょう？

1. エンティティや値オブジェクトにも適さないドメインのビジネスロジック　　
1. ドメインモデル上のタスクの調整に使うロジック（腐敗防止層の変換・アダプターなど）

### クイズ3

![quiz](https://user-images.githubusercontent.com/49358142/185802054-e47c2200-32ce-451f-9fba-8506680997c3.png)

灰色の枠には以下の言葉が入ります。それぞれの言葉は、1,2,3のどこに当てはまるでしょうか？
